'''
@author:
Pledge:I pledge my honor that I have abided by the stevens honor system. -ppate78
...
def catenateLoop(str_list):
    '''Same as catenate, returns a string of all the things together'''
    str = ''
    if str_list == []:
        return ''
    for x in str_list:
...
def wordScoreLoop(S, scorelist):
    ...
    for x in range(len(S)):
        score+=letterScoreLoop(S[x],scorelist)
    return score
